...
def get_embeddings_from_kg_fast(kgpath, modelpath, base_url=None,
   ...
   # entities = [x.strip() for x in entities]
    ...
    print('predict embeddings')
    if base_url != None:
        entity_embs = bert.get_embeddings([urljoin(base_url, e) for e in
                                           entities])
    else:
        entity_embs = bert.get_embeddings(entities)
    # predicate_embs = bert.get_embeddings(predicates)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bert-path", help="Path to the folder with "
                                            "containing the trained BERT "
                                            "model",
                                            type=str,
                        )
